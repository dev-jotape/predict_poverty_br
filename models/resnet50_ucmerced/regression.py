...
# ucmerced = pd.DataFrame(np.load('../../dataset/features/resnet50_ucmerced/features_with_city_code.npy'))
# eurosat = pd.DataFrame(np.load('../../dataset/features/resnet50_eurosat/features_with_city_code.npy'))
# bigearth = pd.DataFrame(np.load('../../dataset/features/resnet50_bigearth/features_with_city_code.npy'))
# imagenet = pd.DataFrame(np.load('../../dataset/features/vgg16_imagenet/features_with_city_code.npy'))
# y_all = np.load('../../dataset/features/resnet50_ucmerced/population.npy')
y_all = np.load('../../dataset/features/resnet50_ucmerced/income.npy')

# print(imagenet.shape)
# print(ucmerced)
# print(eurosat)
# print(bigearth)

# merge = pd.merge(left=ucmerced, right=eurosat, left_on=2048, right_on=2048, how='left')
# merge = pd.merge(left=merge, right=bigearth, left_on=2048, right_on=2048, how='left')
# merge = pd.merge(left=merge, right=imagenet, left_on=2048, right_on=25088, how='left')
# x_all = merge.loc[:, ~merge.columns.isin([2048])].to_numpy()

# print(merge)
# print(merge.shape)
# print(bigearth.shape)
# print(merge.shape)
