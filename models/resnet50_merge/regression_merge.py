...
y_all = np.load('../../dataset/features/resnet50_imagenet/income.npy')

features_image = pd.DataFrame(np.load('../../dataset/features/resnet50_imagenet/features_with_city_code.npy'))
features_finetunin = pd.DataFrame(np.load('../../dataset/features/resnet50_imagenet_finetuning/features_with_city_code.npy'))
...
features_merged = pd.merge(left=features_image, right=features_finetunin, on=100352, how='left')
features_merged = pd.merge(left=features_merged, right=basic_features, left_on=100352, right_on=0, how='left')
...
merge = pd.merge(left=features_merged, left_on=100352, right=lights_grouped, right_on='city_code', how='left')
merge = merge.loc[:, ~merge.columns.isin([100352, 'city_code'])]
merge['income'] = y_all
...
# print(merge)
# exit()
x_all = merge.loc[:,:200725]
y_all = merge.loc[:,200726]
...
best alpha =>  10
best l1_ratio =>  0.15
avg score =>  -119.81121157625061
best position =>  38
mae =>  -119.81121157625061
rmse =>  -163.6962858411396
r2 =>  0.4957192003726011

R2  0.6243923937254641
RMSE  213.9311725995072
MAE  149.68947207121886
'''
